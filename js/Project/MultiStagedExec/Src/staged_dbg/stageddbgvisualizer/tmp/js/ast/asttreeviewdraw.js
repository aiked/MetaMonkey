...
( function( root, factory ) {
	...
	if ( typeof define === 'function' && define.amd ) {
		define( [ 'underscore', 'jquery', 'asttraverse', 'asttreeview', 'asttreestringify', 'exports' ], 
			function( _, $, astTraverse, astView, astStringify, exports ) {
				...
				root.Ast = root.Ast || {};
				root.Ast.DrawTree = exports = factory( root, _, $, astTraverse, astView.GenHtml, astStringify );
			} 
		);
	...
	///////////////////////////////////////////////////////
 	...
 	function drawTree( targetTree, targetSel, opts ) {
		...
		var postProcessNodes = function( root ) {
			calcChildDepth( root );
			...
			var item = node.item
 			...
 			var showSrcCode = ( function() { 
	 			...
	 			return function() {
	 				...
	 				stringify = AstTraverse.unknown( astNode, AstStringify() );
	 				var startPos = 0;
	 				var charsWidth = 0;
		...
		// draw starting point
 		...
 		postProcessNodes( tree.root );
 		var startingX = calcChildMiddleWidth( tree.root.maxChildren );
		startingX += TVD_OPTS.AstViewPadding;
 		drawNode( tree.root, CreatePoint( startingX, TVD_OPTS.AstViewPadding + 20 ) );
