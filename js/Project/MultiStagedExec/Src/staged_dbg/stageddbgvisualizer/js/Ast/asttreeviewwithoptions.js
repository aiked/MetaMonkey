...
( function( root, factory ) {
	...
	if ( typeof define === 'function' && define.amd ) {
		define( [ 'underscore', 'jquery', 'asttraverse', 'asttreeview', 
				...
				root.Ast = root.Ast || {};
				return root.Ast.DrawTreeWithOpt = exports = 
	...
	var drawTreeWithOpt = function( target, ast ) {
		...
		drawOptions = optionGen.drawOptions( target.find( '.astopts' ), opts );
		var stgOpt = drawOptions.get( 'stgoptions' );
		collapseOptions( stgOpt, 'hide' );

		drawTree = astViewDraw.drawTree( domtree,  target.find( '.astview' ), optsValues );
