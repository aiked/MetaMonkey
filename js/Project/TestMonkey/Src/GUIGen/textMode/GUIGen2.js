...
var xrcToAst = {
	...
	parseItems: {
		...
		wxBoxSizer: function(obj, parent){
			...
			var items = "(function(parent){ var divChild = document.createElement('div');" +
			...
			for (var i=0; i<sizeritems.length; ++i) {
				...
				if( sizeritem.type == "element" && sizeritem.name == "object"){
					var item = this.parent.parseSizerItem(sizeritem, 'divChild');
					items += item;
							 			
				}
			}
			items += "parent.appendChild(divChild);})(" + parent + ");" ;
			return items;
		},
		wxTextCtrl: function(obj, parent){
			...
			return "(function(parent){var textarea = document.createElement('textarea');" +
					"textarea.setAttribute('value','"+ getElemValue(value) +"');" +
					"parent.appendChild( textarea );})(" + parent + ");" ;

		},
		wxButton: function(obj, parent){
			var label = getElemByName(obj, "label");
			return "(function(parent){var button = document.createElement('button');" +
		...
		callParser: function(obj, parent){
			...
			return this[classType](obj, parent);
		}
	}, 
	...
	parsePanel: function(obj, parent){
		...
		return "(function(parent){ var divPanel = document.createElement('div');" +
					items +
					"parent.appendChild(divPanel);})(" + parent + ");" ;
	},

	startParser: function(obj){
		this.parseItems.parent = this;
		var items = this.parsePanel(obj, "divroot");
		return "(function(){ var divroot = document.createElement('div');" +
				items + "; return divroot; })();" ;
	}
};

var parsed = xrcToAst.startParser(panelObj);
