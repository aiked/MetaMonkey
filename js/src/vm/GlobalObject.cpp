...
GlobalObject::initFunctionAndObjectClasses(JSContext *cx)
{
    ...
    RootedFunction functionProto(cx);
    {
         ...
         */
        {
            JSObject *proto = NewFunction(cx, false, functionProto, NULL, 0, JSFunction::INTERPRETED,
                                          self, NullPtr());
            if (!proto)
                return NULL;
    ...
    RootedFunction objectCtor(cx);
    {
        ...
        objectCtor = NewFunction(cx, false, ctor, obj_construct, 1, JSFunction::NATIVE_CTOR, self,
                                 objectAtom);
        if (!objectCtor)
            return NULL;
    }
    ...
    RootedFunction functionCtor(cx);
    {
        ...
        functionCtor = NewFunction(cx, false, ctor, Function, 1, JSFunction::NATIVE_CTOR, self,
                                   functionAtom);
        if (!functionCtor)
            return NULL;
     ...
     * in one place.
     */
    RootedFunction getter(cx, NewFunction(cx, false, NullPtr(), ProtoGetter, 0, JSFunction::NATIVE_FUN,
                                          self, NullPtr()));
    if (!getter)
        return NULL;
#if JS_HAS_OBJ_PROTO_PROP
    RootedFunction setter(cx, NewFunction(cx, false, NullPtr(), ProtoSetter, 0, JSFunction::NATIVE_FUN,
                                          self, NullPtr()));
    if (!setter)
        return NULL;
    ...
    RootedFunction throwTypeError(cx, NewFunction(cx, false, NullPtr(), ThrowTypeError, 0,
                                ...
                                gc::AllocKind kind)
{
    ...
    return NewFunction(cx, false, NullPtr(), ctor, length, JSFunction::NATIVE_CTOR, self, name, kind);
}
